...
@Processor(ConversationProcessorConfig.queue_name, {
  concurrency: ConversationProcessorConfig.no_of_jobs,
  limiter: {
    max: ConversationProcessorConfig.max_no_of_job_per_second,
    duration: 1000,
  },
})
@Injectable()
  ...
  implements OnModuleInit, OnModuleDestroy
{
  ...
  private readonly CREATE_BUFFER_KEY = 'buffer:conversation:create';
  private readonly UPDATE_BUFFER_KEY = 'buffer:conversation:update';
  private readonly CREATE_LOCK_KEY = 'lock:conversation:create:flush';
  private readonly UPDATE_LOCK_KEY = 'lock:conversation:update:flush';

  private readonly BATCH_SIZE = ConversationProcessorConfig.batch_size || 100;
  private readonly FLUSH_INTERVAL =
    ConversationProcessorConfig.batch_timeout || 5000;
  ...
  async onModuleInit() {
    this.redis = (await this.conversationQueue.client) as Redis;
    ...
    this.logger.log('ConversationProcessor initialized with Redis batching');
  }
  ...
  private async bufferCreate(data: CreateConversationDto) {
  ...
  private async bufferUpdate(data: UpdateConversationDto) {
    if (!data.id) {
  ...
  private async moveToDLQ(
    ...
    const dlqKey = `dlq:conversation:${operation}`;
